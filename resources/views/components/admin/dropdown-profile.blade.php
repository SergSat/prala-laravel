...
<div class="relative inline-flex" x-data="{ open: false }">
    <button class="inline-flex items-center justify-center group" aria-haspopup="true" @click.prevent="open = !open"
        ...
        <div class="flex items-center truncate">
            <span
                class="ml-2 text-sm font-medium truncate dark:text-slate-300 group-hover:text-slate-800 dark:group-hover:text-slate-200">{{
                Auth::user()->name }}</span>
    ...
    <div class="origin-top-right z-10 absolute top-full min-w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-1.5 rounded shadow-lg overflow-hidden mt-1 {{$align === 'right' ? 'right-0' : 'left-0'}}"
        ...
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200 dark:border-slate-700">
            ...
            <div class="text-xs italic text-slate-500 dark:text-slate-400">{{ __('admin.administrator') }}</div>
        </div>
        <ul>
            <li>
                <a class="flex items-center px-3 py-1 text-sm font-medium text-indigo-500 hover:text-indigo-600 dark:hover:text-indigo-400"
                    ...
                    @focusout="open = false">{{ __('admin.settings') }}</a>
            </li>
            <li>
