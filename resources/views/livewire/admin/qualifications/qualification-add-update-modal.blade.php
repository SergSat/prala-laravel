<x-admin.dialog-modal wire:model.live="show">
    <x-slot name="title">
        {{ __('admin.' . ($id ? 'edit_qualification' : 'add_qualification') ) }}
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div class="mb-4">
            <label for="imagePath" class="cursor-pointer">
                @if ($imagePath)
                    <img src="{{ $imagePath->temporaryUrl() }}" class="rounded-lg h-40 object-cover">
                @elseif($currentImagePath)
                    <img src="{{ asset('storage/'.$currentImagePath) }}" class="rounded-lg h-40 object-cover">
                @else
                    <div class="rounded-lg h-40 bg-gray-200 flex justify-center items-center">
    ...
    <x-slot name="footer">
        ...
        @if ($id)
            <x-admin.button :color="'info'" type="submit" wire:click="updateQualification({{$id}})">{{ __('admin.save') }}</x-admin.button>
        @else
