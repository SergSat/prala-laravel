...
<article class="grid w-full grid-flow-row grid-cols-1 gap-5 lg:pt-24">
    ...
    <section class="flex flex-col items-center w-full mx-auto md:items-start">
        ...
        <div class="flex items-center self-start gap-2">
            <a href="{{ route('home-hr') }}" class="inline-flex items-center group">
                ...
                <span class="ml-2 text-lg group-hover:opacity-60">Кабінет HR</span>
            </a>
            <div class="h-full border-l border-black"></div>
            <h3 class="text-xl text-pr-blue">Перелік усього персоналу з контактами</h3>
        </div>
        {{-- Link to Manager Page --}}

        {{-- Staff total List --}}
        <div class="max-w-screen-xl p-5 bg-white mt-7 lg:p-8 lg:mt-5 rounded-xl drop-shadow-lg">
            <ul class="grid grid-cols-1 gap-5 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                @include('parts.hr-staff-total')
                @include('parts.hr-staff-total')
                @include('parts.hr-staff-total')
                @include('parts.hr-staff-total')
                @include('parts.hr-staff-total')
