...
<article class="grid w-full grid-flow-row grid-cols-1 gap-5 lg:pt-24">
    ...
    <section class="flex flex-col items-center w-full mx-auto md:items-start">
        ...
        <div class="flex items-center self-start gap-2">
            <a href="{{ route('home-manager') }}" class="inline-flex items-center group">
                ...
                <span class="ml-2 text-lg group-hover:opacity-60">Керівник</span>
            </a>
        ...
        {{-- Staff total List --}}
        <div class="max-w-screen-lg p-5 bg-white mt-7 lg:p-8 lg:mt-5 rounded-xl drop-shadow-lg">
            <ul class="grid grid-cols-1 gap-5 xs:grid-cols-2 lg:grid-cols-3">
                @include('parts.staff-total')
                @include('parts.staff-total')
                @include('parts.staff-total')
                @include('parts.staff-total')
                @include('parts.staff-total')
